...
func list_account_ec2(config config.Config, account_name string, account_id string, result map[string][]string) map[string][]string {
...
func list_organization_ec2(config config.Config) {
	...
	for account_name, account_id := range list_accounts {
		list_ec2 = list_account_ec2(config, account_name, account_id, list_ec2)
	}
